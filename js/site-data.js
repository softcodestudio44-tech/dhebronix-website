// ===== DHEBRONIX - FIREBASE SITE DATA (FINAL FIXED) =====
...
async function loadHomePageData() {
    try {
        ...
        console.log('Using default home page content');
    }
}
...
async function loadEquipmentPageData() {
    try {
        ...
        if (available.length === 0) {
            grid.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:#888;padding:40px;">No equipment available.</p>';
            return;
        }

        grid.innerHTML = available.map((item, index) => {
            // FIX: Properly get image from Firebase data
            let imageUrl = 'https://via.placeholder.com/400x300/1a1a1a/8B1A1A?text=No+Image';
            ...
            return `
                <div class="product-card" data-category="${item.category || 'other'}" data-index="${index}">
                    <div class="product-image" onclick="openEquipmentModal(${index})">
                        <img src="${imageUrl}" 
                             alt="${item.name}" 
                             loading="lazy"
                             onerror="this.onerror=null;this.src='https://via.placeholder.com/400x300/1a1a1a/8B1A1A?text=${encodeURIComponent(item.name)}';">
                        <span class="product-badge ${conditionClass}">${item.condition ? item.condition.toUpperCase() : 'NEW'}</span>
                        <div class="product-actions">
                            <a href="https://wa.me/${whatsappNum}?text=${whatsappMsg}" target="_blank" class="product-inquiry" onclick="event.stopPropagation()">
        ...
        let html = `
            <article class="blog-card featured">
                <div class="blog-image">
                    <img src="${featured.image || 'https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?w=800'}" alt="${featured.title}" loading="lazy">
            ...
            html += rest.map(post => {
                ...
                return `
                    <article class="blog-card">
                        <div class="blog-image">
                            <img src="${post.image || 'https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?w=600'}" alt="${post.title}" loading="lazy">
    ...
    try {
        ...
        grid.innerHTML = team.map(member => {
            ...
            const imageHTML = hasImage
                ? `<img src="${member.image}" alt="${member.name}" loading="lazy" onerror="this.parentElement.innerHTML='<div style=\\'width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#8B1A1A,#C41E3A);font-size:60px;font-weight:700;color:white;font-family:Orbitron,sans-serif;\\'>${initials}</div>'">`
...
async function loadSiteSettings() {
    try {
        const [settings, social] = await Promise.all([
            dbGetSettings('company'),
            dbGetSettings('social')
        ]);
